...
export default function locateLibreWolf (
  ...
  const envPath = e?.LIBREWOLF_BINARY
...
const DEFAULT_INSTALL_STEPS: InstallGuidanceStep[] = [
  {
    summary: 'Install LibreWolf from the official site',
...
export function getInstallGuidance (opts?: InstallGuidanceOptions): string {
  ...
  const lines = [
    "We couldn't find a LibreWolf browser on this machine.",
    '',
    'To install one:',
    ''
  ]
  ...
  lines.push('')
  lines.push('Alternatively, set LIBREWOLF_BINARY=/path/to/librewolf and re-run.')

  return lines.join('\n')
}

export function locateLibreWolfOrExplain (
  options?: boolean | {allowFallback?: boolean}
): string {
  const allowFallback =
    typeof options === 'boolean' ? options : Boolean(options?.allowFallback)

  const found = locateLibreWolf(allowFallback) || locateLibreWolf(true)
...
/**
 * Cross-platform LibreWolf version resolver.
 ...
 */
export function getLibreWolfVersion (
  bin: string,
